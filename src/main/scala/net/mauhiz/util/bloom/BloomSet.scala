...
object BloomSet {
	...
	def load[A](is: InputStream) = {
		...
		var contents: IndexedSeq[Boolean] = Vector[Boolean]()
		...
		new BloomSet[A](size, k, contents.asInstanceOf[Vector[Boolean]])
	}

	private def convertToBytes(i: Int) = {
		val buf = new Array[Byte](4)
	...
	private def convertToInt(buf: Array[Byte]) = {
		((buf(0) & 0xFF) << 24) |
			((buf(1) & 0xFF) << 16) |
			((buf(2) & 0xFF) << 8) |
			(buf(3) & 0xFF)
	...
	private def hash(e: Any, iters: Int, bounds: Int): Int = {
